...
export const Input: React.FC<InputProps> = observer(({ onChange, renderControls }) => {
  ...
  return (
    <div className="relative flex w-full items-center">
      ...
      />
      <div ref={controlsRef} className="absolute right-0 flex flex-row pr-2.5">
        {renderControls?.()}
        {query ? (
          <div className="ml-3 flex items-center">
            <IconButton onClick={handleIconButtonClick} name="close" transparent paddingless size="s" />
          </div>
        ) : null}
      </div>
    </div>
