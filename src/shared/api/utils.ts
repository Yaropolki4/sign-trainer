...
export function isClientErrorStatus(status: number) {
  return status >= CLIENT_ERROR_LOWER_BOUNDARY && status < CLIENT_ERROR_UPPER_BOUNDARY;
}

export function isServerErrorStatus(status: number) {
  return status >= SERVER_ERROR_LOWER_BOUNDARY && status < SERVER_ERROR_UPPER_BOUNDARY;
}
